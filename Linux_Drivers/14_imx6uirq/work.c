...
struct irq_keydesc{
    ...
    struct tasklet_struct tasklet; /* tasklet结构体 */
    struct work_struct work;     /* work结构体 */
};
...
static irqreturn_t key0_handler(int irq, void *dev_id)
{
   ...
   // tasklet_schedule(&dev->irqkey[0].tasklet); /* 调度tasklet */
   schedule_work(&dev->irqkey[0].work); /* 调度work */
...
static void key_work(struct work_struct *work)
{
   // struct imx6uirq_dev *dev = (struct imx6uirq_dev*)data;
    ...
    mod_timer(&imx6uirq.timer, jiffies + msecs_to_jiffies(20)); /* 20ms定时 */
}
...
static int keyio_init(struct imx6uirq_dev *dev)
{
    ...
    //dev->irqkey[0].tasklet=key0_tasklet;
