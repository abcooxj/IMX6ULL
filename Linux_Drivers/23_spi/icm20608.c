...
static int icm20608_read_regs(struct icm20608_dev *dev, u8 reg, void *buf, int len)
{
    ...
    //**片选拉低 */
    gpio_set_value(dev->cs_gpio, 0);/*拉低片选使icm20608进行通信*/

    //**构建spi_transfer */
    t=kzalloc(sizeof(struct spi_transfer), GFP_KERNEL);/*使用kzalloc函数为spi_transfer结构体分配内存。
                                                        GFP_KERNEL是内存分配的标志，表示在内核空间进行正常分配*/

    //**第一步：发送要读取的地址 */
    txdata[0] = reg | 0x80;/*进行或运算，0X80一般为读取操作*/
    t->tx_buf = txdata;/*设置传输的发送缓冲区为txdata*/
    t->len = 1;/*读取的长度为1*/

    spi_message_init(&m);/*初始化SPI消息m*/
    spi_message_add_tail(t, &m);//!将传输t添加到消息m的尾部*/
                                /*spi_transfer 添加到 spi_message 队列中*/
    ret=spi_sync(spi,&m);//!同步发送SPI消息，进行实际的SPI传输*/

    //**第二步：读取数据 */
    txdata[0] =0xff;    /*无效的*/
    t->rx_buf =buf;/*设置接收缓冲区为buf*/
    t->len=len;

    spi_message_init(&m);/*重新初始化SPI消息m，并将传输t添加到消息m的尾部*/
    spi_message_add_tail(t, &m);
    ret=spi_sync(spi,&m);/*再次同步发送SPI消息，进行实际的SPI传输，并将返回值存储在ret中*/
...
static int icm20608_read_regs(struct icm20608_dev *dev, u8 reg, void *buf, int len)
{
    ...
    //!因为cs-gpios即是软件片选，也是硬件片选，所以不需要拉低
    /* 片选拉低 */
    //gpio_set_value(dev->cs_gpio, 0);
    ...
    //??不使用这两个函数控制的原因是，使用示波器看波形的时候会发现，片选信号拉低后，表示数据传输完成之后，片选信号会自动拉高
    //??而是在写完一个字节后会突然拉低再后拉高，然后又拉低再发送后面的，这样是不对的，这样会使时序紊乱，id读出0xff
    //spi_write(spi, &data, 1); /* 发送要读取的寄存器地址 */
    //spi_read(spi, buf, len);/* 读取数据 */

    //!因为cs-gpios即是软件片选，也是硬件片选，所以不需要拉高
    //gpio_set_value(dev->cs_gpio, 1); /* 片选拉高 */
    return 0;
}
...
static int icm20608_write_regs(struct icm20608_dev *dev, u8 reg, u8 *buf, int len)
{
    ...
    txdata = kzalloc(len + 1, GFP_KERNEL);/*写的寄存器地址要和数据放在一起，所以len+1*/
...
//!!!!这段不用的原因是cs-gpios即是硬件片选意思软件片选，所以不需要再获取片选 */
#if 0
   //**获取片选引脚 */
   /*获取片选引脚的作用是在SPI（串行外设接口）通信中，通过控制片选引脚的电平来选择与哪个外设进行通信。*/
   /* 获取片选引脚 */
    icm20608dev.nd = of_get_parent(spi->dev.of_node);
    icm20608dev.cs_gpio = of_get_named_gpio(icm20608dev.nd, "cs-gpios", 0);
