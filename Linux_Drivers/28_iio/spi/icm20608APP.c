...
// 调用函数 file_data_read 读取 file_path 数组中索引为 index 的文件内容，并将结果存入 str 字符串中，同时返回值存入 ret
// 使用标准库函数 atof 将字符串 str 转换为浮点数，
#define SENSOR_FLOAT_DATA_GET(ret, index, str, member)\
	ret = file_data_read(file_path[index], str);\
	dev->member = atof(str);\
	
/* 字符串转数字，将整数字符串转换为整数数值 */
// 使用标准库函数 atoi 将字符串转换为整数
...
static int file_data_read(char *filename, char *str)
{
    ...
    ret = fscanf(data_stream, "%s", str); // 使用 fscanf 读取文件中的字符串到 str 中
    if(!ret) {
        printf("file read error!\r\n");
    } else if(ret == EOF) {
        /* 读到文件末尾的话将文件指针重新调整到文件头 */
        fseek(data_stream, 0, SEEK_SET); // 根据文件指针的位置和偏移量来定位文件指针。(只用来：定位！！！）
    }
