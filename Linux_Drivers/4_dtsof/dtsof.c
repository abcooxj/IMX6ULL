...
static int __init dtsof_init(void)
{
    ...
    //**4.获取数组类型的属性 */
    elemseize = of_property_count_elems_of_size(bl_nd, "brightness-levels", sizeof(u32));
    if(elemseize < 0) 
    {
        ret=-EINVAL;
        goto fail_readele;
    ...
    //**申请内存 */
    brival = kmalloc(elemseize * sizeof(u32), GFP_KERNEL);
    if (!brival) {
        ret = -EINVAL;
	    goto fail_mem;
    }
    ...
    //**获取数组 */
    ret=of_property_read_u32_array(bl_nd,"brightness-levels",brival,elemseize);
    if(ret<0)
    {
        goto fail_readarray;
    }
