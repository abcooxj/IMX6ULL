    ...
    , ui(new Ui::Widget)
{
    ...
    // 获取当前目录
    QString currentDir = QDir::currentPath();//静态获取工作目录，存储到currentDir
    // 拼接photo文件夹路径
...
void Widget::on_pushButton_photos_clicked()
{
    if(0 == v4l2_close()){
        timer->stop();
    }
...
void Widget::on_pushButton_take_clicked()
{
   ...
   buffer.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;//视频捕获
