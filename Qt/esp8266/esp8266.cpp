...
Esp82266::Esp82266(QWidget *parent)
{
    ...
    if (!serialPort->open(QIODevice::ReadWrite))
        qDebug()<<"串口无法打开！可能正在被使用！22"<<endl;
    else {
        qDebug()<<"串口打开成功！11"<<endl;
    }
...
void Esp82266::serialPortReadyRead()
{
    ...
    if (readData.contains("OK") && readData.contains("AT+CWMODE")) {
        qDebug()<<"设置STA模式成功33"<<endl;
        sendCmdToEsp8266("AT+CWJAP=\"104\",\"104104104\"");
        qDebug()<<"开始连接WIFI44"<<endl;
        readData.clear();
    }

    if  (temp.contains("WIFI GOT IP")) {
        qDebug()<<"连接WIFI成功55"<<endl;
        sleep(2);
        /* 原子云的设备号及密码 */
        sendCmdToEsp8266("AT+ATKCLDSTA=\"42786873551582757464\",\"12345678\"");
        qDebug()<<"开始连接原子云请等待66"<<endl;
    }

    if (temp.contains("CLOUD CONNECTED")) {
        qDebug()<<"连接原子云成功77"<<endl;
        sleep(2);
        /* 15s就发送一次心跳包 */
        timer->start(15000);
    }
